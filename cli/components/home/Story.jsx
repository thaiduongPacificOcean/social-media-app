    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.storyContainer}>
      ...
      </View>
      {
        data?.map((userStory, index) => (
          userStory._id != user._id && (
            <View key={index}>
              <TouchableOpacity>
                <View style={styles.imgStory}>
                  <Image style={styles.story}
                    placeholder={blurhash}
                    source={{ uri: userStory.img ? userStory.img : DEFAULT_IMG }}
                    contentFit="cover"
                    transition={1000}
                  />
                </View>
                <Text style={styles.strName}>
                  {userStory.username.length > 10 ? userStory.username.slice(0, 9).toLowerCase() + '...'
                    : userStory.username.toLowerCase()}
                </Text>
              </TouchableOpacity>
            </View>
          )
